...
def calculate_scaled_laplacian(adj_mx, lambda_max=2.0):
    ...
    return normalized_laplacian.toarray()  # 返回密集矩阵
...
def calculate_random_walk_matrix(adj_mx):
    ...
    return random_walk_mx.toarray()  # 返回密集矩阵
...
class DCGRUCell(nn.Module):
    ...
    def _calculate_chebyshev_polynomials(self, support, x, K=None):
        ...
        x0 = x
        x1 = torch.matmul(support, x0)  # 使用普通矩阵乘法
        supports.append(x1)
        
        for k in range(2, K + 1):
            x2 = 2 * torch.matmul(support, x1) - x0  # 使用普通矩阵乘法
            supports.append(x2)
            x1, x0 = x2, x1
...
class DCRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # 堆叠输出: (num_nodes, seq_len, output_dim)
