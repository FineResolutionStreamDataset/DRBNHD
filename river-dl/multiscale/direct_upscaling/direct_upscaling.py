...
from multiscale.direct_upscaling.predict import predict_from_io_data
from multiscale.direct_upscaling.torch_utils import train_torch
...
#pre-train
#data = np.load(f"../{outdir}/dataset/prepped_CRW_NHD_pretrain.npz")  #这里与微调不同 直接使用了CRW的NHD
...
for partition in partitions:
    outfile = f"your path to river-dl/results/results_direct_upscaling/{MODEL_NAME}/preds/{basin}_{maskpercentage_formatted}_{partition}_{model_seed}_preds.feather"
    ensure_dir(outfile)
...
for metric_type in metric_types:
    ...
    outfile = f"your path to river-dl/results/results_direct_upscaling/{MODEL_NAME}/metrics/{metric_type}/{basin}_{maskpercentage_formatted}_{model_seed}_metrics.csv"
    ensure_dir(outfile)
    combined_metrics(obs_file=config['obs_file_NHD'],
                     ...
                     pred_trn=f"your path to river-dl/results/results_direct_upscaling/{MODEL_NAME}/preds/{basin}_{maskpercentage_formatted}_trn_{model_seed}_preds.feather",
                     pred_val=f"your path to river-dl/results/results_direct_upscaling/{MODEL_NAME}/preds/{basin}_{maskpercentage_formatted}_val_{model_seed}_preds.feather",
                     pred_tst=f"your path to river-dl/results/results_direct_upscaling/{MODEL_NAME}/preds/{basin}_{maskpercentage_formatted}_tst_{model_seed}_preds.feather",
