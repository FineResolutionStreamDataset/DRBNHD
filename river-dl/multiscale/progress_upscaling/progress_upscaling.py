...
merged_df = merged_df.groupby(['date', 'COMID']).first().reset_index()
...
prepped_NHD = prep_all_data(
              ...
              spatial_idx_name='COMID',
              catch_prop_file=None,
              train_start_date=config['train_start_date_long'],
              train_end_date=config['train_end_date_long'],
              val_start_date=config['val_start_date_long'],
              val_end_date=config['val_end_date_long'],
              test_start_date=config['test_start_date_long'],
              test_end_date=config['test_end_date_long'],
              #segs=None,
              segs=getSegs('COMID'),
...
for partition in partitions:
    outfile = f"your_path_to_river-dl/results/results_progress_upscaling/{MODEL_NAME}/preds/{basin}_{maskpercentage_formatted}_{partition}_{model_seed}_preds.feather"
    ensure_dir(outfile)
...
for metric_type in metric_types:
    ...
    outfile = f"your_path_to_river-dl/results/results_progress_upscaling/{MODEL_NAME}/metrics/{metric_type}/{basin}_{maskpercentage_formatted}_{model_seed}_metrics.csv"
    ensure_dir(outfile)
    combined_metrics(obs_file=config['obs_file_NHD'],
                     ...
                     pred_trn=f"your_path_to_river-dl/results/results_progress_upscaling/{MODEL_NAME}/preds/{basin}_{maskpercentage_formatted}_trn_{model_seed}_preds.feather",
                     pred_val=f"your_path_to_river-dl/results/results_progress_upscaling/{MODEL_NAME}/preds/{basin}_{maskpercentage_formatted}_val_{model_seed}_preds.feather",
                     pred_tst=f"your_path_to_river-dl/results/results_progress_upscaling/{MODEL_NAME}/preds/{basin}_{maskpercentage_formatted}_tst_{model_seed}_preds.feather",
