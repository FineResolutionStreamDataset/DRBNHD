...
def mypredict_from_io_data_remap(
        model,
        io_data_x,
        io_data_x2,
        io_data_y,
...
def predict(
    ...
    np.save("D:/river/river-dl/workflow_examples",y_pred) #2023
...
def predict_torch(x_data,x_data2, model, batch_size, batch_size2):
    ...
    for (x, x2) in zip(dataloader, dataloader2):
        ...
        with torch.no_grad():
            out0 = model(trainx)
            b, t, f = out0.shape
            out0_reshaped = out0.view(b, -1)
            out0_reshapped1 = torch.matmul(model.remap_matrix, out0_reshaped)
            output = out0_reshapped1.view(model.remap_matrix.shape[0], t, f)
            output = output.cpu()  # Move to CPU before appending
